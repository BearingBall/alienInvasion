...
inline void LevelEditor::setTexts(sf::RenderWindow& window)
{
...
inline std::string LevelEditor::tokenToString(wallType token)
{
...
inline void LevelEditor::setCurrentTypeLabel()
{
	ButtonFabric fabric(font);
	std::function<void(Object&)> nothing([](Object& object)
	{
...
inline void LevelEditor::buttonOperator(sf::RenderWindow& window, Vector2 mouse)
{
	if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
	{
		if (!differentClickLBM)
		{
			...
			if (currentType == wallType::doorIn)
			{
				...
				std::for_each(level.getWalls().begin(), level.getWalls().end(), [&possibleToAdd](const Wall& object)
				{
					if (object.type == wallType::doorIn) possibleToAdd = false;
				});
			}
			if (currentType == wallType::doorOut)
			{
				...
				std::for_each(level.getWalls().begin(), level.getWalls().end(), [&possibleToAdd](const Wall& object)
				{
					if (object.type == wallType::doorOut) possibleToAdd = false;
				});
			}
	...
	{
		std::vector<Wall>::iterator tmp = std::find_if(level.getWalls().begin(), level.getWalls().end(),
		                                               [&mouse](const Wall& object)
		                                               {
			                                               return object.model.ifOnMouseOver(mouse);
		                                               });
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Q))
	{
		if (!differentClickQ)
		{
			...
			else if (currentType == wallType::spawnPoint)
			{
				currentType = wallType::doorIn;
			}
			else if (currentType == wallType::doorIn)
			{
				currentType = wallType::doorOut;
			}
			else if (currentType == wallType::doorOut)
			{
