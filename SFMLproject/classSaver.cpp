...
void ClassSaver<Level>::save(size_t numberLevel, Level& level)
{
	...
	for (size_t i = 0; i < level.getWalls().size(); ++i)
	{
		...
		saveFile[std::string("level.walls[") + std::to_string(i) + "].coordinate.x"] = level.getWalls()[i]
		                                                                               .getCoordinate().x;
		saveFile[std::string("level.walls[") + std::to_string(i) + "].coordinate.y"] = level.getWalls()[i]
		                                                                               .getCoordinate().y;
...
void ClassSaver<Level>::download(size_t numberLevel, Level& level)
{
	nlohmann::json saveFile;
