...
public:
	...
	{
		VectorInt newStartPoint = { static_cast<size_t>(player.getCoordinate().x / wallSize),
	...
	{
		VectorInt current = { static_cast<size_t>((enemy.getCoordinate().x + enemy.modelBound().x / 2) / wallSize),
		...
		if (point(current.x, current.y - 1) < point(current.x, current.y))
		{
